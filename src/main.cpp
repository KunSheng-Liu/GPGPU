...
void parser_cmd (int argc, char** argv) 
{
    ...
    for (int i = 1; i < argc;)
    {
        ...
        else if (flag == "-S" || flag == "--sm-dispatch") 
        {
            try{
                string option = argv[i++];
                if (option == "Baseline")      command.SM_MODE = SM_DISPATCH::Baseline;
        ...
        else if (flag == "-T" || flag == "--test-set") 
        {
            ...
            std::cout << "\t-S, " << std::left << setw(20) << "--sm-dispatch"      << "Baseline | Equal | SMD | R_SMD"   << std::endl;
            std::cout << "\t-M, " << std::left << setw(20) << "--mem-allocate"     << "None | Average | MEMA | R_MEMA"   << std::endl;
            std::cout << "\t-T, " << std::left << setw(20) << "--test-set"         << "Light | Heavy | Mix | ALL | LeNet | ResNet18 | VGG16 | GoogleNet"  << std::endl;
